...
#################################
# 最新接口
# jinpan_url = "http://172.19.133.29:8026/getSocialFeaturesBins?identityNo=%s"

# jinpan_url = "http://172.19.133.30:8127/getSocialFeaturesBins?identityNo=%s"
jinpan_url = "http://127.0.0.1:5000/getSocialFruadScore?identityNo=%s"

# jinpan_url = "http://172.19.133.29:8024/getSocialFeatures?identityNo=%s&create_time=%s"


df_all = pd.read_csv("data/20191127185228tj_2019_int_sample_pro.csv")
# df_all = pd.read_csv("data/2018tmp_jk.txt")
df_all = df_all.head(250000)
...
def save_feature(range_index):
    ...
    f = open(write_folder+"social_emer_2019_score_online.csv" ,'w')
    ...
    for index in range_index:
        ...
        m1_times = df_all.loc[index, "m1_times"]
        m2 = df_all.loc[index, "m2"]
        m3 = df_all.loc[index, "m3"]
        m4 = df_all.loc[index, "m4"]
        m5 = df_all.loc[index, "m5"]
        ...
        try:
            ...
            feat_title.append('m1_times')
            feat_value.append(str(m1_times))
            feat_title.append('m2')
            feat_value.append(str(m2))

            feat_title.append('m3')
            feat_value.append(str(m3))
            feat_title.append('m4')
            feat_value.append(str(m4))
            feat_title.append('m5')
            feat_value.append(str(m5))
